...
def download_file_from_ftp(remote_filename, local_path):
    """Download a file from the FTP server to a local path, forcing passive mode."""
    try:
        ...
        print(f"Permission/connection error downloading {remote_filename}: {e}")
...
def download_all_files():
    ...
    for product_type, file_info in ftp_files.items():
        ...
        # Verify that the file actually arrived and is non-empty
        lp = file_info["local_path"]
        if not os.path.exists(lp) or os.path.getsize(lp) == 0:
            print(f"⚠️ WARNING: {lp} is missing or empty!")
...
class DiamondCatalogProcessor:
    ...
    def process_file(self, file_path, product_type):
        ...
        product_templates = {
            "natural": {
                ...
                "description": lambda row: (
                    f"Discover sustainable luxury with our natural {row['shape']} diamond: {row['carats']} carats, "
                    f"{row['col']} color, and {row['clar']} clarity. Perfect for custom creations. "
                    f"Measurements: {row['length']}-{row['width']}x{row['height']} mm. "
                    f"Cut: {row['cut']}, Polish: {row['pol']}, Symmetry: {row['symm']}, "
                    f"Table: {row['table']}%, Depth: {row['depth']}%, Fluorescence: {row['flo']}. "
                    f"{row['lab']} certified {row['shape']}"
                ),
            ...
            "lab_grown": {
                ...
                "description": lambda row: (
                    f"Discover sustainable luxury with our lab-grown {row['shape']} diamond: {row['carats']} carats, "
                    f"{row['col']} color, and {row['clar']} clarity. Perfect for custom creations. "
                    f"Measurements: {row['length']}-{row['width']}x{row['height']} mm. "
                    f"Cut: {row['cut']}, Polish: {row['pol']}, Symmetry: {row['symm']}, "
                    f"Table: {row['table']}%, Depth: {row['depth']}%, Fluorescence: {row['flo']}. "
            ...
            "gemstone": {
                ...
                "description": lambda row: (
                    f"{row['shape']} {row['gemType']} {row['Color']} Gemstone - {row['carats']} carats, "
                    f"color: {row['Color']}, clarity: {row['Clarity']}, cut: {row['Cut']}, "
                    f"lab: {row['Lab']}, treatment: {row['Treatment']}, "
                    f"origin: {row['Mine of Origin']}, size: {row['length']}x{row['width']} mm."
                ),
        ...
        df['google_product_category'] = [random.choice([
            189, 190, 191, 197, 192, 194, 6463, 196, 200, 5122,
            5123, 7471, 6870, 201, 502979, 6540, 6102, 5996, 198, 5982
        ...
        return df[[
            ...
            'average_review_rating', 'number_of_ratings', 'condition'
        ]]
